...
public class MiscStuffForge {
    private static final Logger LOGGER = LogManager.getLogger("EKOTSK Forge");
    public static void init(IEventBus bus) {
        bus.addListener(MiscStuffForge::onCommonSetup);
        MinecraftForge.EVENT_BUS.addListener(MiscStuffForge::onToolModification);
        MinecraftForge.EVENT_BUS.addListener(MiscStuffForge::onReloadListeners);
    }
    static void onToolModification(BlockEvent.BlockToolModificationEvent event) {
        ...
        if (state.is(ModBlocks.WEIRWOOD_WOOD.get())) {
            BlockState stripped = ModBlocks.STRIPPED_WEIRWOOD_WOOD.get()
                    .defaultBlockState();
