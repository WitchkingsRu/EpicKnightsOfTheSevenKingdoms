...
public class WeirwoodBowItem extends BowItem implements IHasModelProperty
{
    ...
    public WeirwoodBowItem(Properties properties, float arrowSpeed, float pullTime)
    {
        super(properties);
    ...
    public void releaseUsing(ItemStack p_40667_, Level p_40668_, LivingEntity p_40669_, int p_40670_)
    {
        if (p_40669_ instanceof Player)
        {
            ...
            if (!itemstack.isEmpty() || flag)
            {
                ...
                {
                    itemstack = new ItemStack(ModItems.WEIRWOOD_ARROW.get());
                }

                float f = getPower(i);
                if (!((double)f < 0.1D))
                {
                    boolean flag1 = player.getAbilities().instabuild || (itemstack.getItem() instanceof WeirwoodArrowItem && EnchantmentHelper.getItemEnchantmentLevel(Enchantments.INFINITY_ARROWS, p_40667_) > 0);
                    if (!p_40668_.isClientSide)
                    {
                        WeirwoodArrowItem arrowitem = (WeirwoodArrowItem)(itemstack.getItem() instanceof WeirwoodArrowItem ? itemstack.getItem() : ModItems.WEIRWOOD_ARROW.get());
                        WeirwoodArrowEntity abstractarrow = arrowitem.createArrow(p_40668_, itemstack, player);
